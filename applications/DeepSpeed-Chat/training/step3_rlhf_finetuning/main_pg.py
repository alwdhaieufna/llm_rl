...
from ppo_trainer import DeepSpeedPPOTrainer, DeepSpeedPPOTrainerUnsupervised, DeepSpeedPGTrainer
from rlhf_engine import DeepSpeedRLHFEngine, DeepSpeedRLHFPGEngine
...
def main():
    ...
    supervised_training_enabled = args.supervised_training_enabled
    ...
    rlhf_engine = DeepSpeedRLHFPGEngine(
    ...
    ppo_trainer = DeepSpeedPPOTrainerUnsupervised if unsupervised_training_enabled else DeepSpeedPGTrainer
...
#            print(batch_prompt)
            ...
            if exp_dataset is not None:
                inner_iter = 0
                actor_loss_sum, critic_loss_sum, unsup_loss_sum, sup_loss_sum = 0, 0, 0, 0
                average_reward = 0
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        if supervised_training_enabled:
                            sup_loss = trainer.train_supervised(
                                exp_data, args.sup_coef)
                ...
                print_rank_0(
                    f'epoch: {epoch}|step: {step}|ppo_ep: {ppo_ep+1}|act_loss: {actor_loss_sum/inner_iter}|cri_loss: {critic_loss_sum/inner_iter}|unsuper_loss: {sup_loss_sum/inner_iter}',
                    args.global_rank)
